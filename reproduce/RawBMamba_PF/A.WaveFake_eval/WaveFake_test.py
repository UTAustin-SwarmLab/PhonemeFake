...
def main():
    parser = argparse.ArgumentParser(description = 'WaveFake evaluation')
    ...
    parser.add_argument("--wav_path", type=str,
                        help="wav file path",
                        required=True, default=None)
    parser.add_argument("--model_folder", type=str, help="model folder", required=False, default='./models')
    ...
    eval_set=Dataset_ASVspoof2021_eval(list_IDs = file_eval, base_dir = args.wav_path)
    device = "cuda"
    dtype = torch.float32
    mamba_config = MambaConfig()
    model = MambaLMHeadModel(config=mamba_config, device=device, dtype=dtype).to(device)
    #for epoch in range(1, 33):
    #model.load_state_dict(torch.load(os.path.join(args.model_folder, 'checkpoint','senet_epoch_%d.pt' % epoch)))
    model.load_state_dict(torch.load(os.path.join(args.model_folder, 'rawbmamba_v2.pt'), map_location='cuda:0'))
